...
export class Puzzle {
  ...
  make_frameline() {
    ...
    for (var j = 0; j < this.centerlist.length; j++) {
      ...
      for (var i = 0; i < corner; i++) {
        max = Math.max(
          this.point[this.centerlist[j]].surround[i],
          this.point[this.centerlist[j]].surround[(i + 1) % corner]
        );
        min = Math.min(
          this.point[this.centerlist[j]].surround[i],
          this.point[this.centerlist[j]].surround[(i + 1) % corner]
  ...
  gridspace_calculate() {
    ...
    for (var i = 0, len = data.length; i < len; i += 4) {
      var j =
        ((i / 4) % this.canvas.height) * this.canvas.width * 4 +
  ...
  make_solution() {
    ...
    for (var i in this[pu].number) {
      ...
      if (
        !isNaN(this[pu].number[i][0]) ||
        !this[pu].number[i][0].match(/[^A-Za-z]+/)
      ) {
        if (
          this[pu].number[i][1] === 2 &&
  ...
  maketext_ppfile() {
    ...
    if (!isEmpty(this.pu_q.surface)) {
      ...
      for (var j = 2; j < this.ny0 - 2; j++) {
        for (var i = 2; i < this.nx0 - 2; i++) {
          if (
            this.pu_q.surface[i + j * this.nx0] &&
  ...
  undo() {
    var a =
      this.pu_q.command_undo.pop(); /*a[0]:list_name,a[1]:point_number,a[2]:value*/
    var pu_mode;
    if (a) {
      if (a[3]) {
        pu_mode = a[3];
    ...
    if (this.mode[this.mode.qa].edit_mode === "number") {
      ...
      if (str_num.indexOf(key) != -1) {
        if (this[this.mode.qa].symbol[this.cursol]) {
          if (
            this[this.mode.qa].symbol[this.cursol][0] === parseInt(key, 10) &&
  ...
  re_specialup(num, arr) {
    if (
      this[this.mode.qa][arr].slice(-1)[0] &&
  ...
  re_polygondown(num) {
    ...
    if (!this.drawing) {
      ...
      if (
        num != this[this.mode.qa][arr].slice(-1)[0][0] &&
        num !=
          this[this.mode.qa][arr].slice(-1)[0][
            this[this.mode.qa][arr].slice(-1)[0].length - 2
          ]
      ) {
        this[this.mode.qa][arr].slice(-1)[0].push(num);
      } else {
  ...
  re_combi_shaka_move(x, y, num) {
    ...
    if (this.drawing_mode === 1) {
      ...
      if (
        this[this.mode.qa].symbol[this.last] &&
  ...
  re_combi_battleship(num) {
    if (
      ...
      for (var i = 0; i < 4; i++) {
        if (
          this[this.mode.qa].symbol[this.point[num].adjacent[i]] &&
  ...
  re_combi_numfl_up(num) {
    if (this.point[num].type === 0 && this.last === num && this.first === num) {
      if (
        !this[this.mode.qa].number[this.last] ||
  ...
  draw_polygonsp(pu) {
    for (var i = 0; i < this[pu].polygon.length; i++) {
      if (this[pu].polygon[i][0]) {
        ...
        this.ctx.beginPath();
        this.ctx.moveTo(
          this.point[this[pu].polygon[i][0]].x,
          this.point[this[pu].polygon[i][0]].y
        );
        for (var j = 1; j < this[pu].polygon[i].length; j++) {
          this.ctx.lineTo(
            this.point[this[pu].polygon[i][j]].x,
            this.point[this[pu].polygon[i][j]].y
          );
        }
        this.ctx.stroke();
        this.ctx.fill();
  ...
  draw_freecircle() {
    /*free_circle*/
    if (
      ((this.mode[this.mode.qa].edit_mode === "line" ||
      ...
      if (this.freelinecircle_g[0] != -1) {
        this.draw_circle(
          this.ctx,
          this.point[this.freelinecircle_g[0]].x,
          this.point[this.freelinecircle_g[0]].y,
          0.3
        );
      }
      if (this.freelinecircle_g[1] != -1) {
        this.draw_circle(
          this.ctx,
          this.point[this.freelinecircle_g[1]].x,
          this.point[this.freelinecircle_g[1]].y,
          0.3
        );
      }
    }
  }
  draw_circle(ctx: any, x: any, y: any, arg3: number) {
    throw new Error("Method not implemented.");
  }
  ...
  draw_cursol() {
    ...
    if (
      ...
      if (
        ...
        this.draw_polygon(
          this.ctx,
          this.point[this.cursolS].x,
          this.point[this.cursolS].y,
          0.2,
          4,
          45
        );
        ...
        this.draw_polygon(
          this.ctx,
          this.point[this.cursol].x,
          this.point[this.cursol].y,
          0.2,
          4,
          45
        );
        ...
        this.ctx.beginPath();
        this.ctx.moveTo(
          this.point[this.point[this.cursol].surround[0]].x,
          this.point[this.point[this.cursol].surround[0]].y
        );
        for (var j = 1; j < this.point[this.cursol].surround.length; j++) {
          this.ctx.lineTo(
            this.point[this.point[this.cursol].surround[j]].x,
            this.point[this.point[this.cursol].surround[j]].y
          );
        }
        this.ctx.closePath();
        this.ctx.stroke();
