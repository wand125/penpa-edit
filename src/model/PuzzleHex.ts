...
export class Puzzle_hex extends Puzzle {
  constructor(nx, ny, size) {
    ...
    this.center_n = Math.floor(
      (this.nx * 3 + 1) ** 2 * 0.5 + this.nx * 3 * 0.5 * (this.nx % 2)
    );
  ...
  key_arrow(key_code) {
    var a, b, c;
    if (Math.floor(this.theta / 60) === 0) {
      b = [0, 1, 2, 3];
    } else if (Math.floor(this.theta / 60) === 1) {
      b = [3, 0, 1, 2];
    } else if (Math.floor(this.theta / 60) === 2) {
      b = [2, 3, 0, 1];
    } else if (Math.floor(this.theta / 60) === 3) {
      b = [2, 3, 0, 1];
    } else if (Math.floor(this.theta / 60) === 4) {
      b = [1, 2, 3, 0];
    } else if (Math.floor(this.theta / 60) === 5) {
      b = [1, 2, 3, 0];
    }
    if (this.reflect[0] === -1) {
      c = b[0];
    ...
    switch (num) {
      ...
      /*//for square
                case 5:
                  set_circle_style(ctx,10);
                  ctx.beginPath();
                  ctx.moveTo(x+0.07*this.size,y+0.2*this.size);
                  ctx.lineTo(x-0.07*this.size,y+0*this.size);
                  ctx.lineTo(x+0.07*this.size,y-0.2*this.size);
                  ctx.stroke();
                  break;*/
    }
  }
  ...
  draw_pencils(ctx, num, x, y) {
    ...
    switch (num) {
      case 1:
        ctx.beginPath();
        ctx.moveTo(x + 0.5 * this.size, y - 0.5 * this.size);
        ctx.lineTo(x, y);
        ctx.lineTo(x + 0.5 * this.size, y + 0.5 * this.size);
        ctx.closePath();
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x + r * this.size, y - r * this.size);
