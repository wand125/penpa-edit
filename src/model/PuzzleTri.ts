...
export class Puzzle_tri extends Puzzle {
  n0: number;
  top_n: number;
  constructor(nx, ny, size) {
    ...
    this.nx = nx;
    this.ny = ny;
    this.n0 = Math.floor((this.nx * 4) / 3 + 4);
    this.margin = 10;
    this.size = size;
    this.space = [parseInt(document.getElementById("nb_space1")['value'], 10)];

    this.top_n = Math.floor(this.n0 ** 2 + this.n0 * 2.5) - 1;
  ...
  create_point() {
    ...
    for (var j = 0; j < n; j++) {
      for (var i = 0; i < n; i++) {
        ...
        point[k] = new Point(
          (i + (j % 2) * 0.5 - (1 + 0.5 * ((this.nx + 1) % 2))) * this.size,
          (j - 1) * this.size * Math.sqrt(3) * 0.5,
          type,
          adjacent,
          surround,
          use
    ...
    for (var j = 0; j < n; j++) {
      for (var i = 0; i < n; i++) {
        ...
        point[k] = new Point(
          point[i + j * n].x,
          point[i + j * n].y + (2 / 3) * this.size * Math.sqrt(3) * 0.5,
          type,
          adjacent,
          surround,
          use,
    ...
    for (var j = 0; j < n; j++) {
      for (var i = 0; i < n; i++) {
        ...
        point[k] = new Point(
          point[i + j * n].x - 0.5 * this.size,
          point[i + j * n].y + (1 / 3) * this.size * Math.sqrt(3) * 0.5,
          type,
          adjacent,
          surround,
          use,
    ...
    for (var j = 0; j < n; j++) {
      for (var i = 0; i < n; i++) {
        ...
        point[k] = new Point(
          point[i + j * n].x - 0.25 * this.size,
          point[i + j * n].y + this.size * Math.sqrt(3) * 0.25,
          type,
          adjacent,
          surround,
          use,
    ...
    for (var j = 0; j < n; j++) {
      for (var i = 0; i < n; i++) {
        ...
        surround = [];
        point[k] = new Point(
          point[i + j * n].x - r * this.size,
          point[i + j * n].y - r * this.size * Math.sqrt(3),
          type,
          adjacent,
          surround,
          use
    ...
    for (var j = 0; j < n; j++) {
      for (var i = 0; i < n; i++) {
        ...
        surround = [];
        point[k] = new Point(
          point[i + j * n].x - r * this.size * Math.sqrt(3),
          point[i + j * n].y - r * this.size,
          type,
          adjacent,
          surround,
          use
  ...
  key_arrow(key_code) {
    ...
    if (
      ...
      if (Math.floor(this.cursol / this.n0 ** 2) === 1) {
        switch (c) {
          case 0:
      ...
      } else if (Math.floor(this.cursol / this.n0 ** 2) === 2) {
        switch (c) {
          case 0:
            a = this.point[this.cursol].adjacent[1];
            if (this.point[a].use === 1) {
          ...
          switch (direction) {
            ...
            case 180:
              this.ctx.text(
                ...
                this.point[i].y + 0.1 * this.size,
                this.size * fontsize
              );
              this.ctx.beginPath();
              this.ctx.arrow(
                this.point[i].x + (arrowlength * 0.5 + 0.0) * this.size,
                this.point[i].y + (arrowlength * 0.0 - 0.15) * this.size,
                this.point[i].x + (-arrowlength * 0.5 + 0.0) * this.size,
                this.point[i].y + (-arrowlength * 0.0 - 0.15) * this.size,
                [0, 1, -0.15 * this.size, 1, -0.15 * this.size, 3]
              );
            ...
            case 150:
              ...
              this.ctx.arrow(
                this.point[i].x +
                  (arrowlength * 0.25 * Math.sqrt(3) + 0.05) * this.size,
                this.point[i].y + (arrowlength * 0.25 - 0.12) * this.size,
                this.point[i].x +
                  (-arrowlength * 0.25 * Math.sqrt(3) + 0.05) * this.size,
                this.point[i].y + (-arrowlength * 0.25 - 0.12) * this.size,
                [0, 1, -0.15 * this.size, 1, -0.15 * this.size, 3]
            ...
            case 90:
              ...
              this.ctx.beginPath();
              this.ctx.arrow(
                this.point[i].x + (arrowlength * 0.0 + 0.15) * this.size,
        ...
        case "4": //tapa
          ...
          if (this[pu].number[i][0].length === 1) {
            ...
            this.ctx.text(
              this[pu].number[i][0].slice(0, 1),
              this.point[i].x - 0.0 * this.size,
              this.point[i].y - 0.13 * this.size,
              this.size * 0.8
            );
            this.ctx.text(
              this[pu].number[i][0].slice(1, 2),
              this.point[i].x - 0.15 * this.size,
          ...
          if (sum === 1) {
            ...
            for (var j = 0; j < 9; j++) {
              if (this[pu].number[i][0][j] === 1) {
                this.ctx.text(
                  (j + 1).toString(),
                  this.point[i].x + ((j % 3) - 1) * 0.15 * this.size,
                  this.point[i].y +
  ...
  draw_cross(ctx, num, x, y) {
    for (var i = 0; i < 6; i++) {
      if (num[i] === 1) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          x + ((ctx.lineWidth * Math.sqrt(6)) / 6) * Math.cos(th),
          y + (ctx.lineWidth ** Math.sqrt(3) / 6) * Math.sin(th)
        );
        ctx.lineTo(
          x - (Math.sqrt(3) / 6) * pu.size * Math.cos(th),
          y - (Math.sqrt(3) / 6) * pu.size * Math.sin(th)
        );
        ctx.stroke();
      }
    }
