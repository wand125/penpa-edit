...
export class Puzzle_pyramid extends Puzzle {
  top_n: number;
  constructor(nx, ny, size) {
    ...
    this.top_n = Math.floor(this.nx0 * 2.5);
  ...
  draw_lattice() {
    if (this.mode.grid[1] === "1") {
      ...
      for (var i = 0; i < this.centerlist.length; i++) {
        ...
        if (
          this.centerlist.indexOf(this.point[this.centerlist[i]].surround[0]) !=
            -1 ||
        ...
        if (
          this.centerlist.indexOf(this.point[this.centerlist[i]].surround[4]) !=
            -1 ||
